...
const storage=(()=>{
  try{
    ...
    return {getItem:k=>memoryStore.has(k)?memoryStore.get(k):null,setItem:(k,v)=>memoryStore.set(k,String(v)),removeItem:k=>memoryStore.delete(k),key:i=>Array.from(memoryStore.keys())[i]??null,get length(){return memoryStore.size}};
  }
})();
...
const state={
 ...
 pinHash:storage.getItem("cs2.pinHash")||"",
 query:"",
 filter:"All"
};

const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
function read(k,d){try{return JSON.parse(storage.getItem(k))??d}catch{return d}}
function save(){storage.setItem("cs2.bookmarks",JSON.stringify(state.bookmarks));storage.setItem("cs2.history",JSON.stringify(state.history));storage.setItem("cs2.progress",JSON.stringify(state.progress));storage.setItem("cs2.collections",JSON.stringify(state.collections));storage.setItem("cs2.downloads",JSON.stringify(state.downloads));storage.setItem("cs2.extensions",JSON.stringify(state.extensions))}
...
function renderSearch(q=state.query){
 ...
 const grouped=arr.reduce((a,x)=>{(a[x.provider]??=[]).push(x);return a},{});
...
function pinModal(){
 ...
 $("#modal").hidden=false;$("#savePin").onclick=async()=>{const v=$("#pin").value;if(!/^\d{4,8}$/.test(v))return alert("Use 4–8 digits.");state.pinHash=await hashPin(v);storage.setItem("cs2.pinHash",state.pinHash);$("#modal").hidden=true;};
}
async function lock(){
...
$("#wipe").onclick=()=>{if(confirm("Delete ALL local CS 2.0 data?")){for(const k of Object.keys(localStorage))if(k.startsWith("cs2."))storage.removeItem(k);storage.removeItem("cs2.pinHash");location.reload()}};
